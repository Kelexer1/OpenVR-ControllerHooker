...
// Additions
vr::HmdVector3_t rotateVectorByQuaternion(const vr::HmdVector3_t& v, const vr::HmdQuaternion_t& q) {
	// v' = q * v * q^-1
	float num = q.x * 2.f;
	float num2 = q.y * 2.f;
	float num3 = q.z * 2.f;
	...
	vr::HmdVector3_t result;
	result.v[0] = (1.0f - (num5 + num6)) * v.v[0] + (num7 - num12) * v.v[1] + (num8 + num11) * v.v[2];
	result.v[1] = (num7 + num12) * v.v[0] + (1.0f - (num4 + num6)) * v.v[1] + (num9 - num10) * v.v[2];
	result.v[2] = (num8 - num11) * v.v[0] + (num9 + num10) * v.v[1] + (1.0f - (num4 + num5)) * v.v[2];
	return result;
}