...
int main() {
	...
	SharedControllerMemoryIO sharedMemory(&eventListener);
	...
	ControllerCommandSender commandSender(&sharedMemory);
	...
	for (uint32_t deviceIndex : controllerIndexes) {
		commandSender.setUsePingPong(deviceIndex, true);
		// ControllerHooker will now automatically call IControllerEventListener::poseRequested
		// We implement the IControllerEventListener class through ControllerEventListener.h and .cpp
